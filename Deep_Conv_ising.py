...
def imparo_ising(n=35, ntrain=18, nval=10, ntest=4, momento=0.8, read_file=True):
    """
    Convolutional Neural network made up of FullyConnected layers and Convolutional layers
    ...
    print(data.shape)
    data = data.reshape((len(data), n, n))  # Reshape in 2D
    data_val = data_val.reshape((len(data_val), n, n))
    data_test = data_test.reshape((len(data_test), n, n))
    print(data.shape)

    """####################################### Build and train my network ########################################"""
    # change the hidden layer size and study the effect
    # or add more layers. for example add Conv2D(2, (3, 3), data.shape[1], 1), activation="relu") after the first one..
    rete = keras.models.Sequential([
        Conv2D(2, (2, 2), input_shape=(data.shape[1], data.shape[1], 1), activation="relu"),
        Flatten(),
